        ...
        for mi=1:repeat_m %looping over all semantic meaning grid points
            ...
            for ls =1:length(lsteps)-1 %now for each centre of the stimuli the range of observations is looked at
                for ms =1:length(msteps)-1
                    L_av_xa(ms,ls,t)=L_av_xa(ms,ls,t)+normpdf(lsteps(ls),L_av_s,sig_la);
                    ...
                    %search).
                    L_av_xv(ms,ls,t)=L_av_xv(ms,ls,t)+normpdf(lsteps(ls),L_av_s,sig_lv);
                    ...
                    M_av_xa(ms,ls,t)=M_av_xa(ms,ls,t)+normpdf(msteps(ms),M_av_s,sig_ma);
                    ...
                    M_av_xv(ms,ls,t)=M_av_xv(ms,ls,t)+normpdf(msteps(ms),M_av_s,sig_mv);
...
%                     for ls =1:length(lsteps)-1 %now for each centre of the stimuli the range of observations is looked at
%                         for ms =1:length(msteps)-1
%                             L_av_xa_c2(ls,ms,t)=L_av_xa_c2(ls,ms,t)+(normcdf(lsteps(ls+1),L_a,sig_la_c2)-normcdf(lsteps(ls),L_a,sig_la_c2));
%                             L_av_xv_c2(ls,ms,t)=L_av_xa_c2(ls,ms,t)+(normcdf(lsteps(ls+1),L_v,sig_lv_c2)-normcdf(lsteps(ls),L_v,sig_lv_c2));
%                             M_av_xa_c2(ls,ms,t)=M_av_xa_c2(ls,ms,t)+(normcdf(msteps(ms+1),M_a,sig_ma_c2)-normcdf(msteps(ms),M_a,sig_ma_c2));
%                             M_av_xv_c2(ls,ms,t)=M_av_xa_c2(ls,ms,t)+(normcdf(msteps(ms+1),M_v,sig_mv_c2)-normcdf(msteps(ms),M_v,sig_mv_c2));
%                             %the above calcualtes the spread of the observations
%                             %given each of the locations for the centres in the
%                             %grid. By summing the probabilities distributions of
%                             %the observations, for all possible stimulus locations
