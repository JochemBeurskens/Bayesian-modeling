...
n=50; %maybe not discritize, set the number of possible locations, the larger the more accurate, but also the more computation time is required
m_n=4;
...
%                 product(:)=(prod(prob_ax_t(:,:),2));
            product_p_ax(l_x_i,m_x_i)=(prod(prod(prob_ax_t(:,:),2)));
...
%         prob_ax_s=sum(product_s);
        prob_ax(l_s_i,m_s_i)=sum(sum(prob_l'.*prob_m.*product_p_ax));    
    end
end
% hold off;
if m_n==1 || n==1
