...
for t=1:t_max
    ...
    %     mi=1;
        for mi=1:repeat_m
            M=normrnd(-0.5,sig_m_s); %drawing the M and L from the probability distributions for these parameters, this should simulate the effect of multiplication with the P(L_{av}^{s}) etc.
            L=normrnd(0.5,sig_l_s);
            M_a=normrnd(0.25,sig_m_as_c2); %drawing the M and L from the probability distributions for these parameters, this should simulate the effect of multiplication with the P(L_{av}^{s}) etc.
            M_v=normrnd(-1.25,sig_m_vs_c2);
            L_a=normrnd(-0.25,sig_l_as_c2);
            L_v=normrnd(1.25,sig_l_vs_c2);
            ...
            for q=1:repeat
                ...
                [i_fax,i_fvx,i_las_plt,i_mas_plt,i_lvs_plt,i_mvs_plt,i_la_plt,i_ma_plt,i_lv_plt,i_mv_plt,i_percept_L_a,i_percept_M_a,i_percept_L_v,i_percept_M_v]=Sample_model(k,1,n,c,l,m,L_a,M_a,sig_e_as_c2,sig_e_vs_c2,sig_l_as_c2,sig_l_vs_c2,sig_m_as_c2,sig_m_vs_c2,sig_e_ax_c2,sig_e_vx_c2,sig_la_c2,sig_ma_c2,sig_lv_c2,sig_mv_c2,L_v,M_v);
...
P_av_given_LMf_collapsedc=zeros(n,n,t_max); 
P_a_given_LMf_collapsedc=zeros(n,n,t_max);
P_v_given_LMf_collapsedc=zeros(n,n,t_max);

P_av_given_LMf_collapsedc(:,:,:)=sum(P_av_given_LMf(:,:,:,:),3)./sum(sum(sum(P_av_given_LMf(:,:,:,:),3)));
P_a_given_LMf_collapsedc(:,:,:)=sum(P_a_given_LMf_c2(:,:,:,:),3)./sum(sum(sum(P_a_given_LMf_c2(:,:,:,:),3)));
P_v_given_LMf_collapsedc(:,:,:)=sum(P_v_given_LMf_c2(:,:,:,:),3)./sum(sum(sum(P_v_given_LMf_c2(:,:,:,:),3)));

% now normalising, so that for each point equations 9 and 10 of Koerding et
% al. 2007 make sense (i.e. the sum of the two should be 1)
Normalised_P_av_a=P_av_given_LMf_collapsedc./(P_a_given_LMf_collapsedc+P_av_given_LMf_collapsedc);
Normalised_P_a_a=P_a_given_LMf_collapsedc./(P_a_given_LMf_collapsedc+P_av_given_LMf_collapsedc);
Normalised_P_av_v=P_av_given_LMf_collapsedc./(P_v_given_LMf_collapsedc+P_av_given_LMf_collapsedc);
Normalised_P_v_v=P_v_given_LMf_collapsedc./(P_v_given_LMf_collapsedc+P_av_given_LMf_collapsedc);
%% calculating the model averaging estimates
S_a=Normalised_P_av_a.*store_percept_av + Normalised_P_a_a.*store_percept_a;
S_v=Normalised_P_av_v.*store_percept_av + Normalised_P_v_v.*store_percept_v;
S_a=S_a./(sum(sum(S_a,'omitnan'),'omitnan'));
S_v=S_v./(sum(sum(S_v,'omitnan'),'omitnan'));

[x,y]=meshgrid(l,m);
figure(1)
contour(x,y,S_a(:,:,1),'ShowText','on')
hold on;
xlabel("Location")
ylabel("Semantic meaning")
figure(2)
contour(x,y,S_a(:,:,2),'ShowText','on')
hold on;
xlabel("Location")
ylabel("Semantic meaning")
figure(3)
contour(x,y,S_a(:,:,3),'ShowText','on')
hold on;
xlabel("Location")
ylabel("Semantic meaning")
figure(4)
contour(x,y,S_v(:,:,1),'ShowText','on')
hold on;
xlabel("Location")
ylabel("Semantic meaning")
figure(5)
contour(x,y,S_v(:,:,2),'ShowText','on')
hold on;
xlabel("Location")
ylabel("Semantic meaning")
figure(6)
contour(x,y,S_v(:,:,3),'ShowText','on')
hold on;
xlabel("Location")
ylabel("Semantic meaning")
...
figure(1)
contour(x,y,P_av_given_LMf_collapsedc(:,:,1),'ShowText','on')
hold on;
xlabel("Location")
ylabel("Semantic meaning")
figure(2)
contour(x,y,P_av_given_LMf_collapsedc(:,:,2),'ShowText','on')
hold on;
xlabel("Location")
ylabel("Semantic meaning")
figure(3)
contour(x,y,P_av_given_LMf_collapsedc(:,:,3),'ShowText','on')
hold on;
xlabel("Location")
ylabel("Semantic meaning")
