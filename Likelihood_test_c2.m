...
for t=1:t_max
    ...
    %     mi=1;
        ...
        L_v=normrnd(-3,sig_l_vs);
        for mi=1:repeat_m
            repeat=10000;
            M=normrnd(2,sig_m_as); %drawing the M and L from the probability distributions for these parameters, this should simulate the effect of multiplication with the P(L_{av}^{s}) etc.
...
%                 [i_fax,i_fvx,i_l_plt_stat,i_m_plt_stat,i_la_plt_stat,i_ma_plt_stat,i_lv_plt_stat,i_mv_plt_stat]=Likelihood_given_input(k,t_max,n,c,l,m,L,M,sig_e_s,sig_l_s,sig_m_s,sig_e_ax,sig_e_vx,sig_la,sig_ma,sig_lv,sig_mv);
                [i_fax,i_fvx,i_las_plt,i_mas_plt,i_lvs_plt,i_mvs_plt,i_la_plt,i_ma_plt,i_lv_plt,i_mv_plt]=Likelihood_given_input(k,1,n,c,l,m,L,M,sig_e_as,sig_e_vs,sig_l_as,sig_l_vs,sig_m_as,sig_m_vs,sig_e_ax,sig_e_vx,sig_la,sig_ma,sig_lv,sig_mv,L_v,M_v);
...
[x,y]=meshgrid(l,m);
figure(1)
contour(x,y,P_av_given_LMf(:,:,3,1),'ShowText','on')
hold on;
xlabel("Location")
