...
for i=2:t_max
    L_xa(i)=normrnd(c_av_s(i-1),sig_lax); %creating the parameter that relates the noisy percept to the stimulus
    phi_xav(i,:,1)=normrnd(phi_xav(i-1,:,1),sig_lax,1,n);
    L_xv(i)=normrnd(c_av_s(i-1),sig_lvx); %creating the parameter that relates the noisy percept to the stimulus
