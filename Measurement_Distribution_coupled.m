...
n_lm=5; %number of sets of L^x and M^x that are considered for the integrals
...
prob_ax=zeros(n,2,t_max,n_lm,n_r);
prob_vx=zeros(n,2,t_max,n_lm,n_r);
...
for o=1:n_lm
    for p=1:n_r
        for t =1:t_max
            prob_Lvx=normpdf(l,l(i_l),sig_lvx);
            prob_vx(:,:,t,o,p)=prob_Lvx'.*normpdf(v_x(:,:,t,o,p),mean_rw,sig_e_s).*p_c;
            ...
            prob_Lax=normpdf(l,l(i_l),sig_lax);
            prob_ax(:,:,t,o,p)=p_c.*prob_Lax'.*normpdf(a_x(:,:,t,o,p),mean_rw,sig_e_s);
...
xlabel('Location'), ylabel('Meaning'), zlabel('sum of P(a^{x})')
figure(2)
surf(sum(sum(prob_vx(:,:,:,:,:),5),4))
xlabel('Location'), ylabel('Meaning'), zlabel('sum of P(v^{x})')